...
- **N07 — Sync memo local → Lume REMOTE (thủ công)** (`nfr`): memo do người gõ trong app local (kho `memos-md/`) được **đẩy lên một instance Lume ở xa** khi người dùng **bấm** (không tự động, không nền). Idempotent: chạy 2 lần không đẻ bản trùng. **Memo từ mount (llmwiki) KHÔNG được đẩy** — llmwiki là nguồn chân lý local, không phải nội dung của Lume.
