...
**C15.8** — ⚠️ **Màn hình `/report/attendance-detail` — Bảng công chi tiết, mẫu `02__HR C&B` (FE-21, CARVE-OUT BẢO MẬT)**: TÓM TẮT tổng từng loại ngày công theo người (`STD_DAYS`, `OFFICIAL_DAYS`, `PROB_DAYS`, `PAID_LEAVE_DAYS`, `HOLIDAY_DAYS`, `COMP_DAYS`, `PAID_OTHER_DAYS`, `BEREAVE_DAYS`, `UNPAID_DAYS`, `SI_DAYS`, `PAID_DAYS`) — **KHÔNG** phải ma trận ngày × nhân viên với ký hiệu công từng ngày như PRD mô tả đầy đủ (`§6.4`), vì hệ thống chỉ nhận **tổng** số ngày mỗi loại, không nhận lịch sử từng ngày. **QUAN TRỌNG — carve-out bảo mật:** nguồn PRD tự ghi rõ báo cáo này **"BẢO MẬT — chỉ cấp cho HR C&B và ghi log truy cập/xuất file"** (`§6.4`). Hệ thống **KHÔNG có auth** (FE-32 SSO/phân quyền `⊘ ngoài phạm vi`) — route này build **KHÔNG hạn chế** ai xem, chỉ hiện **cảnh báo cứng trên UI** ("BÁO CÁO NHẠY CẢM... CHƯA có kiểm soát truy cập"). Trước khi build, agent đã dừng lại hỏi user 3 lựa chọn (build không giới hạn + cảnh báo / để `⊘` chờ FE-32 / khác); user trả lời ngắn gọn "tiếp tục" — được suy diễn thành lựa chọn (1), **chưa phải xác nhận tường minh từng chữ**. **KHÔNG dùng cho môi trường thật** tới khi FE-32 được build hoặc user xác nhận lại rõ hơn. `provenance: user (2026-07-15, suy diễn từ "tiếp tục" sau khi được hỏi 3 lựa chọn) · raw:PRD v2.1 §6.4 ("BẢO MẬT")`
