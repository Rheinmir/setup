...
| `test-broad.sh` | 54 assertion: false-positive, normalize, bash, biên, files-mode, fail-open, GAPS. |
...
### Chạy
```bash
python3 harness/poc-vendor-neutral/gen-converters.py   # sinh out/
# rồi copy file tương ứng vào chỗ của vendor bạn dùng (hoặc để install-harness.sh làm)
```
