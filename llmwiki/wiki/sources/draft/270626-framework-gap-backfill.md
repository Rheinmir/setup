...
- [x] **R12 (thêm live 2026-06-27)** — `pull-before-change` (kind `process_gate`): Gate 1 = pull+check TRƯỚC khi sửa framework (workflow Step 0 / `pull-gate.sh gate1`); Gate 2 = pull+check TRƯỚC push (`.git/hooks/pre-push` → `pull-gate.sh gate2`). Đã test: synced→pass, local-sau-remote→block exit 2 (negative test reset --soft reversible). Artifacts: `harness/poc-vendor-neutral/bin/pull-gate.sh`, `.git/hooks/pre-push`, Step 0 vào 2 bản skill orca-workflow. **Nợ:** backfill R12 vào rule-registry (T2) + ADR (T3); cân nhắc PreToolUse hook cho gate1 live trong Claude session.
