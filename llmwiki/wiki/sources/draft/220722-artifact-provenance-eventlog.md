...
**FR-005**: Hệ thống PHẢI cung cấp một truy vấn tương quan CODE-change ↔ DOCS-change: hai sự kiện có `topic` khác nhau (`code.change`/`docs.change`) TRONG cùng `writer_id` (hoặc cùng `task_id` nếu có) và cách nhau dưới một ngưỡng thời gian (mặc định — xem Assumptions) PHẢI được đề xuất là liên quan; vượt ngưỡng hoặc mơ hồ PHẢI trả về "cần agent phán đoán" thay vì tự động khẳng định.
...
- [ ] **T4 — `correlate()` trong adapter: đề xuất cặp code.change/docs.change theo độ gần thời gian + cùng writer_id/task_id; trả "cần agent phán đoán" khi mơ hồ.** Verify: SC-003.
