...
| [120926-reprise-prd-work-continuity](sources/120926-reprise-prd-work-continuity.md) | source | Reprise PRD Work Continuity: 16 bất biến state (op_key idempotent, CAS, generation, lease→unknown, provenance mới được "đã xác minh") đã rút ra và áp vào orca-graph; nói rõ phần không vay (SQLite, quota, connector) |
| [120926-reprise-graph-engine-prd](sources/120926-reprise-graph-engine-prd.md) | source | Reprise Graph Engine PRD: ba cấu trúc tách nhau (cây chứa / DAG / state machine), leaf contract, replan giữ lịch sử, control, 12 bất biến — bảng ánh xạ từng mục vào orca-graph v2 và phần ngoài phạm vi |
| [120926-orca-graph-v2-PLAN](sources/draft/120926-orca-graph-v2-PLAN.md) | draft | PLAN 7 task áp Reprise Graph Engine PRD vào orca-graph (cấp chứa, cycle xuyên graph, lint leaf, plan_version, control, atlas theo cấp, SKILL) — đã chạy trọn qua chính orca-graph, 13/13 test |
...
| [110926-downstream-layout-awareness-PLAN](sources/draft/110926-downstream-layout-awareness-PLAN.md) | draft | PLAN 6 task chống ảo giác layout repo↔máy khách: test hook thật trong fixture dot, lint bare-path ratchet, CI downstream theo layout, bản đồ downstream đầu phiên |
