...
| **Skills**               | codify project knowledge         | [Agent Skills](https://developers.openai.com/codex/skills) ([`SKILL.md`](http://SKILL.md)), invoked with `$name` or implicitly                                                 | [Agent Skills](https://addyosmani.com/blog/agent-skills/) ([`SKILL.md`](http://SKILL.md)) |
...
| **State**                | track what’s done                | Markdown or Linear via a connector                                                                                                                                             | Markdown ([`AGENTS.md`](http://AGENTS.md), progress files) or Linear via MCP              |
...
A skill is how you stop re-explaining the same project context every session like a goldfish. Both tools use the same format, a folder with a [`SKILL.md`](http://SKILL.md) inside holding instructions and metadata, and then optional scripts, references, assets. Codex runs a skill when you call it with `$` or `/skills`, or by itself when your task matches the skill description, wich is the reason a tight boring description beats a clever one. Claude Code does it the same way and I wrote the pattern up in [agent skills](https://addyosmani.com/blog/agent-skills/).
...
I made this case twice already, once as [the code agent orchestra](https://addyosmani.com/blog/code-agent-orchestra/) and again in [agentic code review](https://addyosmani.com/blog/agentic-code-review/). The reason it matters specifically inside a loop is the loop runs while you are not watching, so a verifier you actually trust is the only reason you can walk away. Subagents do burn more tokens since each one does its own model and tool work, so spend them where a second opinion is worth paying for. This is also basically what Claude Code’s `/goal` does under the hood, a fresh model decides if the loop is done instead of the one that did the work, the maker and checker split applied to the stop condition itself.
