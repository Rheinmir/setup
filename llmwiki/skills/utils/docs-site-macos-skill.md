...
## Flip-Card Grid (skill / feature catalog)

When a section lists many short items that each have a **front face** (what it is / purpose) and a **back face** (how it works / steps) — e.g. a catalog of skills, features, commands, or team roles — use a **3D flip-card grid** instead of stacked cards. Click a card to flip it; front shows the one-line purpose, back shows the numbered flow. Color-code by group with a `gtag` chip + a legend above the grid.

Auto-fits as many columns as the width allows (`auto-fill, minmax(244px,1fr)`); collapses to fewer columns on narrow screens with no media query needed.

```css
/* skill grid — flip cards */
.sk-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(244px,1fr));gap:14px;margin-top:24px}
.sk-card{perspective:1200px;cursor:pointer;min-height:186px}
.sk-inner{position:relative;width:100%;height:100%;min-height:186px;
  transition:transform .55s cubic-bezier(.4,0,.2,1);transform-style:preserve-3d}
.sk-card.flipped .sk-inner{transform:rotateY(180deg)}
.sk-face{position:absolute;inset:0;display:flex;flex-direction:column;gap:8px;overflow:auto;
  backface-visibility:hidden;-webkit-backface-visibility:hidden;          /* hide the face turned away */
  border:1px solid var(--border);border-radius:15px;
  box-shadow:var(--edge-hi),0 4px 18px rgba(0,0,0,.055);padding:16px;
  backdrop-filter:blur(var(--blur-2)) saturate(1.1);-webkit-backdrop-filter:blur(var(--blur-2)) saturate(1.1)}
.sk-front{background:var(--glass-2)}                                       /* tier-2: lighter front */
.sk-back{background:var(--glass-3);transform:rotateY(180deg)}             /* tier-3: calmer back (more text) */
.sk-card:hover .sk-face{box-shadow:var(--edge-hi),0 10px 28px rgba(0,0,0,.10)}
.sk-card .skh{display:flex;align-items:center;justify-content:space-between;gap:8px}
.sk-card h4{font-family:var(--font-mono);font-size:13.5px;font-weight:600;color:#1d1d1f}
.sk-face .flabel{font-size:9.5px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:#9aa3b2}
.sk-front .flabel{color:#0a84ff}                                          /* purpose label = blue */
.sk-back .flabel{color:#1c8aa0}                                           /* flow label = teal */
.sk-card p{font-size:12px;color:var(--t2);line-height:1.5;flex:1}
.flip-hint{margin-top:auto;font-size:10.5px;color:#9aa3b2;display:flex;align-items:center;gap:5px;align-self:flex-start}
.sk-card .skb{display:flex;flex-direction:column;gap:3px}
.sk-back .step-line{font-size:11.5px;color:var(--t2);line-height:1.55}
.sk-back .step-line b{color:#1c8aa0;font-weight:600}                       /* step number tint */
/* group tags — one tone per category, used on the card + in the legend */
.gtag{font-size:9.5px;font-weight:600;letter-spacing:.02em;padding:2px 8px;border-radius:20px;white-space:nowrap}
.g-wiki{background:rgba(48,176,199,.14);color:#1c8aa0}   /* teal */
.g-dev{background:rgba(88,86,214,.12);color:#5856d6}     /* indigo */
.g-orch{background:rgba(52,199,89,.14);color:#1f9e44}    /* green */
.g-utils{background:rgba(255,149,0,.15);color:#cf7600}   /* orange */
.grp-legend{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px}
```

Markup — flip is a one-liner `onclick="this.classList.toggle('flipped')"` (no JS function needed). The `.sk-back` face MUST also carry `transform:rotateY(180deg)` (in CSS above) so its content reads correctly when flipped into view:

```html
<div class="grp-legend">
  <span class="gtag g-wiki">wiki-loop · nuôi wiki</span>
  <span class="gtag g-dev">dev-loop · code an toàn</span>
  <span class="gtag g-utils">utils · tiện ích nền</span>
</div>

<div class="sk-grid">
  <div class="sk-card" onclick="this.classList.toggle('flipped')"><div class="sk-inner">
    <div class="sk-face sk-front">
      <div class="skh"><h4>ingest</h4><span class="gtag g-wiki">wiki-loop</span></div>
      <span class="flabel">Mục đích</span>
      <p>Biến tài liệu thô thành tri thức wiki dùng được.</p>
      <span class="flip-hint">↻ luồng đi</span>
    </div>
    <div class="sk-face sk-back">
      <div class="skh"><h4>ingest</h4><span class="gtag g-wiki">wiki-loop</span></div>
      <span class="flabel">Luồng đi</span>
      <div class="skb">
        <span class="step-line"><b>1.</b> đọc file mới trong <code>raw/</code></span>
        <span class="step-line"><b>2.</b> phân tích &amp; distill → concepts / entities / sources</span>
        <span class="step-line"><b>3.</b> cập nhật index + log</span>
      </div>
    </div>
  </div></div>
  <!-- …repeat one .sk-card per item… -->
</div>
```

Notes / gotchas:
- **Front and back must repeat the header** (`.skh` with `h4` + `gtag`) so the card identifies itself on both faces.
- **`backface-visibility:hidden` is mandatory** on `.sk-face` — without it the mirrored back face bleeds through the front.
- Keep faces `position:absolute;inset:0` over a `min-height` parent so both stack in the same box; `overflow:auto` on the face lets a long back side scroll instead of clipping.
- Tier the two faces: front = `--glass-2` (lighter, sparse), back = `--glass-3` (calmer, holds more text) — same depth logic as the rest of the system.
- Group-tag tones reuse the Apple secondary palette (teal/indigo/green/orange) so the catalog matches the section color cycle; add a `.g-*` class per category and mirror it in `.grp-legend`.
