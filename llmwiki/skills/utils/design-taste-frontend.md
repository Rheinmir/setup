...
* **Liquid Glass / Glassmorphism:** Appropriate for premium consumer, Apple-adjacent, luxury brand, or media-overlay vibes. Inappropriate for dashboards, public-sector, or "boring B2B." When used, go beyond `backdrop-blur`: add a 1px inner border (`border-white/10`) and a subtle inner shadow (`shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]`) for physical edge refraction. On LARGE panes (sidebars, full-height panels) never use one flat fill alpha — it reads as a milky wall; use a gradient-alpha fill plus a specular-sheen overlay, and put colored background layers directly behind the pane so the blur has something to refract. Provide a solid-fill fallback under `prefers-reduced-transparency`.
