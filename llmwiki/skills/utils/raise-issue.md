...
## Nơi lưu — ledger local, travel được
- Issue = một file draft: `llmwiki/wiki/sources/draft/DDMMYY-<slug>.md` (cùng chỗ propose, đi theo repo khi clone — không phụ thuộc GitHub).
- Index tập trung: `llmwiki/wiki/sources/ISSUES.md` — một dòng/issue (id · tiêu đề · status · assignee), giống MEMORY.md. Đây là "ledger" để quét nhanh issue đang mở. (Lưu ở `sources/` chứ KHÔNG ở `wiki/` root — R5 folder-structure chặn root.)
- Không tạo GitHub issue (quyết định dự án: ledger-in-repo, không lệ thuộc tracker ngoài).
...
6. **Xác nhận, KHÔNG thực hiện**: báo lại issue id + đường dẫn + assignee. Dừng ở đây — việc thực thi là của phiên nhận.
