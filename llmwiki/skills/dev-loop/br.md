...
```
`br-run.py` tự: (1) frame-lint, (2) kiểm working-tree sạch, (3) chạy loop-runner **NGAY TRONG cây hiện tại** với revise **đã trỏ tới `fdk/tools/br-revise.py`** (render `skills/br/assets/revise-prompt.md` → `claude -p` tools bó hẹp), (4) frame xanh → commit vào nhánh hiện tại, message gắn frame_id (mốc revert/blame), (5) in tóm tắt 1 dòng, (6) ghi `run_log_ref` vào frame.
