...
Đây là **điểm bắt đầu duy nhất**. Mở agent (Claude Code · opencode · Cursor · Antigravity…) ở thư mục gốc của dự án mới, rồi **dán nguyên khối giữa hai dấu `---` bên dưới**. Agent sẽ tự cài overstack và dẫn bạn qua 3 pha, dừng hỏi đúng lúc cần — bạn **không** phải copy thư mục hay feed từng file thủ công.
...
Rồi xác nhận đủ 3 trụ đã đúng chỗ: **harness** (validator + hook gác), **skills** (global), **llmwiki** (khung wiki). Nếu dự án đã có `llmwiki/` cũ → gọi `/harness-update` thay vì cài đè.
...
Dựng cấu trúc: `llmwiki/wiki/{concepts,entities,sources/draft}`, `llmwiki/{skills,raw,html}`, file `llmwiki/wiki/index.md` + `llmwiki/wiki/log.md`. Quy tắc bắt buộc: mỗi trang wiki có frontmatter YAML `type:` + section `## Origin` (R2/R9); **không bao giờ ghi vào `raw/`** (R1); file wiki nằm đúng subfolder (R5). Seed một trang `llmwiki/wiki/sources/project-requirements.md` từ câu trả lời PHA 1, cập nhật `index.md` + `log.md`.
