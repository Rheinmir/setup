---
name: harness-update
description: Migrate llmwiki cũ lên harness (case B) hoặc update harness bản mới (case C) — MỘT lệnh `install-harness.sh --self-heal` tự cài + tự backfill nợ wiki (Origin/index/OKF) trong 1 process, không vòng lặp re-run. Trigger: "migrate harness", "update harness", "nâng cấp harness", "cài harness vào dự án cũ", "/harness-update".
---
