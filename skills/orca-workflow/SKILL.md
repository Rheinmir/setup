...
0. **R12 (B) — pre-work sweep (MỘT LẦN, trước khi làm / fan-out đa-agent)**: orchestrator chạy `harness/poc-vendor-neutral/bin/pull-gate.sh gate1` — fetch remote; local SAU/diverge → DỪNG, `git pull --rebase origin <branch>` rồi mới query/propose/dispatch (cả đàn agent chung base tươi). Offline → fail-open. **KHÔNG chặn từng-edit** (cố tình bỏ per-edit PreToolUse). **R12 (C)** trước push tự enforce qua `.git/hooks/pre-push` cho **mọi vendor**; check tay: `pull-gate.sh gate2`.
