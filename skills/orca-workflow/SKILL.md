...
0. **pull-gate R12 — GATE 1 (BẮT BUỘC, TRƯỚC mọi thay đổi framework)**: chạy `harness/poc-vendor-neutral/bin/pull-gate.sh gate1` — fetch remote, nếu local SAU/diverge remote thì DỪNG, `git pull --rebase origin <branch>` rồi mới query/propose/sửa. Offline → fail-open (không khoá cứng). **GATE 2** (trước push) tự enforce bằng `.git/hooks/pre-push`; muốn check tay: `pull-gate.sh gate2`.
