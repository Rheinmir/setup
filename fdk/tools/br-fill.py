...
def framework_assets() -> Path:
    ...
    repo = Path(__file__).resolve().parents[2] / "skills" / "br" / "assets"
    if repo.is_dir():
        return repo
    # Bản CÀI (~/.claude/harness/) không mang theo skills/ — skill nằm ở ~/.claude/skills.
    # Thiếu vế này thì check-contract tung FileNotFoundError, và caller (hoh.py) đọc
    # rc != 0 thành "hợp đồng BR thủng" — hạ tầng hỏng đội lốt phán quyết của cổng.
    return Path.home() / ".claude" / "skills" / "br" / "assets"
