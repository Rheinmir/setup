...
def _default_template() -> Path:
    # Trong repo framework: <repo>/skills/br/assets. Bản CÀI (~/.claude/harness/) không
    # mang skills/ theo — skill nằm ở ~/.claude/skills. Thiếu fallback thì MỌI frame ở
    # G3 chết vì read_text() trên đường dẫn không có.
    repo = Path(__file__).resolve().parents[2] / "skills" / "br" / "assets" / "revise-prompt.md"
    return repo if repo.is_file() else (
