...
> **Đọc trước:** front-door *khái niệm* — pre-flight gate, "không miss rule", "không dẫm module" — nằm ở wiki concept `fdk/wiki/concepts/fdk.md` (gọi nhanh bằng skill `/fdk`). Folder này là phần *vật lý* (tool + tài liệu di động) đi kèm concept đó.
...
|----------|--------|------------|
| Front-door concept (pre-flight, rule, module map) | `fdk/wiki/concepts/fdk.md` | wiki knowledge-graph — cross-link + validate |
| Rule registry R1..Rn | `fdk/wiki/concepts/rule-registry.md` | wiki concept |
| Architecture Decision Records | `fdk/wiki/sources/adr/ADR-*.md` | wiki decision |
...
- Tài liệu thuộc về knowledge-graph (concept/decision) vẫn vào `fdk/wiki/` như cũ; chỉ cần thêm một dòng vào bảng bản đồ trên.
