...
| 2026-06-29 | 2 skill web-crawl + web-clone (BNAL) | skill | user cần crawl site→markdown + clone UI faithful, gọi bằng slash | web-crawl (builtin urllib→md, adapter firecrawl/crawl4ai/jina) + web-clone (inline local→1 file, adapter single-file CLI/monolith); register 4 surface; 17 self-test BNAL; test crawl loop-library OK |
| 2026-06-29 | áp 5 archetype Boris Cherny vào template | architecture | overstack lệch nhịp (Prototyper/Builder nhiều, Sweeper/Maintainer ít); muốn persona dispatch + từ khoá | ADR-015: sweep-gate (nhịp Sweeper, nhắc gọt khi accrete) + archetype.py (phase-map + persona /proto·/build·/sweep·/grow·/maintain inject posture cho CLI) + 5 posture; routing cli verified:false; 19 self-test BNAL |
